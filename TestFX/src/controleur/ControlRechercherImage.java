...
public class ControlRechercherImage {
	...
	public Resultat rechercheChemin(float pourcentageMini, String nom)
    {
        ...
        switch (extension) {
            case "jpg":
                // traitement pour les fichiers jpg
       		 nom = nom.toString().replace(".jpg", ".txt");

                return (rechercheImageCouleur(pourcentageMini, nom));
            case "bmp":
                // traitement pour les fichiers png
          		 nom = nom.toString().replace(".jpg", ".txt");

                return (rechercheImageNB(pourcentageMini, nom));
              default :
            	  return null;
	...
	public Resultat rechercheImageNB(float pourcentageMini, String nom)
	{
		...
		*/
		//nom=rechercheImageNB(pourcentage,nom) du C
		 ...
		 List<String> res = FileHandler.readFileToList("../../pfr_code/data/resultat_recherche.txt");
		 String[] parts = res.get(0).toString().split(" ");
		 String fileName = parts[0];
		 int i =0;
		 for(String s : res){
		 	res.set(i, s.replace(".txt", ".bmp"));
		 	i++;
		 }
		...
		switch (couleur)
		{
			case 1 :
				 ...
				 res.add("28.jpg 1,000%");
			        ...
			        this.resultat =(new Resultat(TypeRechercheCouleur.RGB, res, recherche));
			        bdHistorique.ajouterHistorique(recherche, this.resultat);
			        return resultat;
			case 2:
