...
#include "../include/pile_dynamique.h"
#include "../include/indexation_texte.h"
#include "../include/piles2.h"
#include "../include/table_index.h"
#include "../include/lib_texte.h"

#define CHEMIN_FICHIER_STOPWORD "../../pfr_code/data/stop_words_french.txt"
...
#define CHEMIN_FICHIER_TOK_RECH "../../pfr_code/traitement"
...
#define CHEMIN_FICHIER_OCCURENCE_COMMUN "../../pfr_code/data/OccurenceCommun.txt"
#define CHEMIN_LISTE_RECHERCHE_TRIEE_IDENT "../../pfr_code/data/liste_recherche_triee_ident.txt"
#define CHEMIN_LISTE_RECHERCHE_IDENT "../../pfr_code/data/liste_recherche_ident.txt"
#define CHEMIN_LISTE_RECHERCHE_TRIEE "../../pfr_code/data/liste_recherche_triee.txt"
...
void supprApostrophe(char * pathFile){
    ...
    system("chmod +x ../../pfr_code/texte/src/remove_apostrophe.sh");
    int ret = snprintf(supprApostrophe, sizeof(supprApostrophe), "../../pfr_code/texte/src/remove_apostrophe.sh %sV2.xml", pathFile);
    if (ret < 0) {
        abort();
    }
    else{ 
...
{
    int nb_min;
    FILE *config = fopen("../../pfr_code/config.txt", "r");
...
void creerPileMot(PILE * pileMot , char * fileName, int * nbMotDiff, int ident, char * cheminRepertoireTok){
    ...
    *nbMotDiff = 0;
    ...
    snprintf(recupTokUniq,sizeof(recupTokUniq),"sort %s/%s.tok | uniq > ../../pfr_code/traitement/CloneTok/%sClone.tok",cheminRepertoireTok,fileName,fileName);
    system(recupTokUniq);

    //ouverture du fichier Clone.tok
    snprintf(pathFile,sizeof(pathFile),"../../pfr_code/traitement/CloneTok/%sClone.tok",fileName);
    ...
    FILE * fichier_config = fopen("../../pfr_code/config.txt","r");
...
int chercheNbOccur(char * mot, char * fileName){
    ...
    snprintf(cptnbMot, sizeof(cptnbMot), "grep -ow '%s' ../../pfr/texte/tok/%s.tok | wc -l > ../../pfr_code/data/nbOccur.txt",mot,fileName);
    system(cptnbMot);

    //récupération du nombre d'occurrence du mot dans le fichier nbOccur.txt
    FILE * nbOccur = fopen("../../pfr_code/data/nbOccur.txt","r");
...
int comptageNbMot(char * pathFile){
    ...
    snprintf(comptageMot,sizeof(comptageMot),"wc -w %s > ../../pfr_code/data/comptage.txt",pathFile);
    system(comptageMot);
    
    //récupération du nombre de mot dans le fichier comptage.txt
    FILE * comptage = fopen("../../pfr_code/data/comptage.txt","r");
...
void recherche_comparaison_texte(char * pathFileRecherchee, int nbFichierTxt){
    ...
    snprintf(chaine, 256, "cp %s.xml ../../pfr/texte/recherche_texte > ../../pfr_code/data/erreur.txt 2>&1 ", pathFileRecherchee);
    system(chaine);

    system("echo '' > ../../pfr_code/data/liste_recherche_triee_ident.txt");
    system("echo '' > ../../pfr_code/data/liste_recherche_ident.txt");
    system("echo '' > ../../pfr_code/data/liste_recherche_triee.txt");
    system("echo '' >../../pfr_code/data/resultat_recherche.txt");
    char ecrireResult[256] ={0};

    int ret = snprintf(recupNom,sizeof(recupNom),"basename %s > ../../pfr_code/data/recupNomRech.txt",pathFileRecherchee);
    if(ret<0){
        abort();
    }
    else system(recupNom);

    FILE * recupNomRech = fopen("../../pfr_code/data/recupNomRech.txt","r");
    if(fscanf(recupNomRech,"%s",fileName)==1){
        system("rm -f ../../pfr_code/traitement/*.tok;rm -f ../../pfr_code/traitement/*.clean;");
    ...
    while(fscanf(listeRechTriee,"%s",chemin)==1){
        ...
        */
        int ret = snprintf(ecrireResult,sizeof(ecrireResult),"echo '%s' >> ../../pfr_code/data/resultat_recherche.txt",chemin);
        if(ret<0){
            abort();
        } system(ecrireResult);
    } 
...
void rech_MC(char * mot){
    system("echo >../../pfr_code/data/rech_mot_clef.txt");
    system("echo >../../pfr_code/data/resultat_recherche.txt");
    char ecrireResult[256] ={0};
    char carac[200];
    char chaine[210];
    ...
    while(fscanf(table,"%s",carac)){
        if(strcmp(carac,mot)==0){
            ...
            for(int i=0;i<nbMot;i++){
                fscanf(table,"%d",&fich);
                fscanf(table,"%d",&nb);
                snprintf(chaine, 150,"echo '%d %d'>> ../../pfr_code/data/rech_mot_clef.txt",fich,nb);
                system(chaine);
            }
            snprintf(chaine,270,"sort -k 2 -nr ../../pfr_code/data/rech_mot_clef.txt > ../../pfr_code/data/rech_mot_clef_tri.txt");
            system(chaine);
            FILE *rech = fopen("../../pfr_code/data/rech_mot_clef_tri.txt","r");
            while (fscanf(rech,"%d",&id_fich)==1){
                ...
                while (fscanf(base,"%d",&id_courant) == 1){
                    ...
                    if(id_courant==id_fich){
                        //printf("%s %d\n",chemin,nb);
                        int ret = snprintf(ecrireResult,sizeof(ecrireResult),"echo '%s %d' >> ../../pfr_code/data/resultat_recherche.txt",chemin,nb);
                        if(ret<0){
                            abort();
                        } system(ecrireResult);
                    }
...
void open_fich(){
    ...
    fscanf(rech,"%d",&ident);
    fclose(rech);
    FILE *fich = fopen("../../pfr/texte/descripteurs_textes/Liste_Base_Texte.txt", "r");
...
int comptageNbLigne(char * pathFile){
    ...
    char comptageMot[256]={0};
    //comptage du nombre de mot du fichier passer en paramètre
    snprintf(comptageMot,sizeof(comptageMot),"wc -l %s > ../../pfr_code/data/comptage.txt",pathFile);
    system(comptageMot);
...
int main(int argc, char * argv[]){
    ...
    int ret = snprintf(CommandeRecupNom,sizeof(CommandeRecupNom),"basename %s > ../../pfr_code/data/recupNomRech.txt",argv[1]);
    if(ret<0){
        abort();
    }
    else system(CommandeRecupNom);

    FILE * recupNom = fopen("../../pfr_code/data/recupNomRech.txt","r");
    ...
    ident = comptageNbLigne("../../pfr/texte/descripteurs_textes/Liste_Base_Texte.txt");
    descript = crea_descript_txt(fileName,ident,CHEMIN_FICHIER_TOK_IDX);
    concatBaseDescript(descript);
    ajoutListeBaseTxt(pathFileSRC,ident);
    pMotTable = table(pMotTable,ident,fileName);
