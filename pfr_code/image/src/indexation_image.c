...
{ 
  char chaine[256] = {0} ; 
  //chemin actualisé pour prendre dans le répertoire des fichiers et plus dans recherche_image
  int ret = snprintf ( chaine , 256 ,"../../pfr/image/fich_images/txt/%s" , fichier ) ;
  if ( ret < 0 )
       abort(); 
...
//image = nom du fichier  (pas chemin)
...
{    
         ...
         FILE * LBI = NULL ;
         if (d==3){ 
         Descripteur[0]= 3200 + id;
         BDI = openFile("../../pfr/image/descripteurs_images/base_descripteur_image_couleur.txt" ,"a" ) ; 
         LBI = openFile("../../pfr/image/descripteurs_images/base_image_couleur.txt","a" ) ;
          }
         else { 
          Descripteur[0]= 4200 +id; 
          BDI = openFile("../../pfr/image/descripteurs_images/base_descripteur_image_NB.txt" ,"a" ) ;
          LBI = openFile("../../pfr/image/descripteurs_images/base_image_NB.txt","a" ) ;
          }
          mise_a_jour_base_image ( LBI , BDI, image, Descripteur , n , &d) ;
         for (int i = 0 ; i< NBLIG ;i++ )
               free (matrice[i]) ; 
         free(matrice) ;  
         return (Descripteur) ;
...
int main(int argc, char * argv[]){
  ...
  int * descripteur = indexation_recherche(argv[1]);
}
