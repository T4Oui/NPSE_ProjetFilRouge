...
#include <string.h>
#include <math.h>
//#include <../../pfr_code/image/include/lib_image.h>
...
void remplissagetab (TAB* tableau1,int nbdescripteurs,int noiroublanc)

 {
    if(noiroublanc==0)
    {
        FILE* base_descripteur_image = fopen("../../pfr/image/descripteurs_images/base_descripteur_image_couleur.txt","r");
    ...
    {
        FILE* base_descripteur_image = fopen("../../pfr/image/descripteurs_images/base_descripteur_image_NB.txt","r");
        FILE* config = fopen("../../pfr_code/config.txt","r");
        int nb=configurationR(config);
        for(int i=0;i<nbdescripteurs;i++)    
        {
        ...
        {            if(tableau2[t].pourcentage>=pourcentagemini)
            {
                ...
                snprintf(resultat, sizeof(resultat),"echo '%f %s' >> ../../pfr_code/data/resultat_recherche.txt",(tableau2[t]).pourcentage,tableau2[t].fichier);
                system(resultat);
            }
        }
    }
...
int comptageNbLigne(char * pathFile){
 ...
 int nbdenbdescripteurs(int noiroublanc)
 {
    int nbdescripteurs=0;
        if(noiroublanc==0)
    {
        nbdescripteurs = comptageNbLigne("../../pfr/image/descripteurs_images/base_descripteur_image_couleur.txt");
    }
    else
    {
        nbdescripteurs = comptageNbLigne("../../pfr/image/descripteurs_images/base_descripteur_image_NB.txt");
    }
 ...
 void afficher_resultat_couleur(STRUCTPOURC* tableau2, int nbdescripteurs)
    {
    int id_courant=0;
    FILE *fich = fopen("../../pfr/image/descripteurs_images/base_image_couleur.txt", "r");
    char chemin[256] ={0};
    char chaine[256] ={0};
 ...
 void afficher_resultat_noiroublanc(STRUCTPOURC* tableau2, int nbdescripteurs)
    {
    int id_courant=0;
    FILE *fich = fopen("../../pfr/image/descripteurs_images/base_image_noirblanc.txt", "r");
    char chemin[256] ={0};
    char chaine[256] ={0};
    while (fscanf(fich,"%d",&id_courant) == 1)
        {
...
{
    FILE *fich = fopen("../../pfr/image/descripteurs_images/base_image_NB.txt", "r");
    int id_courant=0;
            for(int i=0;i<nbdescripteurs;i++)
            {
                fscanf(fich,"%d",&id_courant);
...
void open_image_coul(char * fichier){
    ...
    int ret = snprintf(changerNom,sizeof(changerNom),"echo '%s' > ../../pfr_code/data/nfich_rech.txt ",fichier);
    if(ret<0){
        abort();
    } else system(changerNom);
    system("sed -i -e 's/txt/jpg/g' ../../pfr_code/data/nfich_rech.txt");
    FILE * fich =fopen("../../pfr_code/data/nfich_rech.txt","r");
    fscanf(fich,"%s",nomfich);
    ret = snprintf(chaine,sizeof(chaine),"display ../../pfr/image/fich_images/%s",nomfich);
    if(ret<0){
        abort();
    } else system(chaine);
    fclose(fich);
...
void open_image_nb(char * fichier){
    ...
    char nomfich[150];
    char changerNom[256]={0};
    int ret = snprintf(changerNom,sizeof(changerNom),"echo '%s' > ../../pfr_code/data/nfich_rech.txt ; sed -i -e 's/txt/bmp/g' ../../pfr_code/data/nfich_rech.txt",fichier);
    if(ret<0){
        abort();
    } else system(changerNom);
    FILE * fich =fopen("../../pfr_code/data/nfich_rech.txt","r");
    fscanf(fich,"%s",nomfich);
    ret = snprintf(chaine,sizeof(chaine),"viewnior ../../pfr/image/fich_images/%s",nomfich);
    if(ret<0){
        abort();
    } else system(chaine);
    fclose(fich);
...
{ 
  char chaine[256] = {0} ; 
  int ret = snprintf ( chaine , 256 ,"../../pfr/image/fich_images/txt/%s" , fichier ) ;
  if ( ret < 0 )
       abort(); 
...
{ 
  char chaine[256] = {0} ; 
  int ret = snprintf ( chaine , 256 ,"../../pfr/image/recherche_image/%s" , fichier ) ;
  if ( ret < 0 )
       abort(); 
...
void indexation_image () { 
 int id =0 ; 
 char nomFichier[200] ;
 system("ls ../../pfr/image/fich_images/txt > ../../pfr_code/data/listeFichierImage.txt " );  //Créé un fichier avec la liste des textes dans le répertoire 
 FILE * listeFichier = openFile("../../pfr_code/data/listeFichierImage.txt","r") ; 
 system("rm ../../pfr/image/descripteurs_images/*.txt");

 while ( fscanf ( listeFichier ,"%s",nomFichier )==1) // Faire l'indexation de tous les fichiers
 { 

 printf ( "../../pfr/image/fich_images/txt/%s\n",nomFichier);
 id++ ;
 FILE* config = openFile("../../pfr_code/config.txt" , "r" ); 
 ...
 BDI = openFile("../../pfr/image/descripteurs_images/base_descripteur_image_couleur.txt" ,"a" ) ; 
 LBI = openFile("../../pfr/image/descripteurs_images/base_image_couleur.txt","a" ) ;
 }
 
 else {  //image en N/B
 Descripteur[0]= 4000 +id; 
 BDI = openFile("../../pfr/image/descripteurs_images/base_descripteur_image_NB.txt" ,"a" ) ;
 LBI = openFile("../../pfr/image/descripteurs_images/base_image_NB.txt","a" ) ;
 }
